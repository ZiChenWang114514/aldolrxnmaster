...
def _fit_et(X, y, n_estimators=200, random_state=42):
    """Lightweight ExtraTrees for null-importance (champion-class, fewer trees)."""
...
def null_importance(X, y, vmask, feat_names):
    ...
    for axis_y, axis in _axis_iter(y):
        ...
        # p-value = fraction of null runs whose importance >= the real importance
...
# --------------------------------------------------------------------------- #
def perm_importance(X, y_axes, vmask, gold, feat_names, splits, scope):
    """Per-axis permutation drop on gold-test (and full-test caveat)."""
    rows = []
    nf = X.shape[1]
    for axis, y in y_axes.items():
        ...
        for tr, te in _tscv_folds(splits, vmask, scope):
            ...
            g_local = gold[te]                       # gold flag within this test fold
            Xte = X[te]; yte = y[te]
...
def _gold_eval_axes(X, y_axes, vmask, gold, splits, scope):
    """Per-axis (full_w, gold_w) gold-test balanced acc — train + eval, no permutation."""
    out = {}
    for axis, y in y_axes.items():
        full, goldr = [], []
        for tr, te in _tscv_folds(splits, vmask, scope):
        ...
        out[axis] = (_wmean(full), _wmean(goldr))
    return out


def group_ablation(X, y_axes, vmask, gold, feat_names, splits, scope):
    groups = _group_columns(feat_names)
    base = _gold_eval_axes(X, y_axes, vmask, gold, splits, scope)
    rows = []
    for gname, idx in groups.items():
        if not idx:
            continue
        keep = [i for i in range(X.shape[1]) if i not in set(idx)]
        sub = _gold_eval_axes(X[:, keep], y_axes, vmask, gold, splits, scope)
        for axis in y_axes:
            rows.append({
...
#  driver
# --------------------------------------------------------------------------- #
def _axis_iter(y_axes):
    for axis in AXES:
        yield y_axes[axis], axis
...
def main():
    ...
    # shared feature matrix (identical across axes); y per axis
    X, _, _, feat_names = prepare_Xy(target_label="label_joint")
    y_axes, vmask_axes = {}, {}
    for axis in AXES:
        _, y, vm, _ = prepare_Xy(target_label=axis)
        y_axes[axis] = y; vmask_axes[axis] = vm
    # common valid mask = rows valid for ALL axes (so the same rows are compared)
    vmask = np.logical_and.reduce([vmask_axes[a] for a in AXES])
    gold = pd.read_csv(FEAT_DIR / "denoise_meta.csv")["gold"].values.astype(bool)
    scope = np.ones(len(X), bool)            # all-aux (the 156d champion's domain)
    splits = load_splits()
    ...
    # --- M1 null importance (label ablation) ---
    print("\n[M1] null-importance (label ablation) ...")
    # y restricted: invalid rows already -1; null_importance filters by axis_y>=0
    y_for_m1 = {a: np.where(vmask, y_axes[a], -1) for a in AXES}
    m1 = null_importance(X, y_for_m1, vmask, feat_names)
    ...
    # --- M2 gold-test permutation importance ---
    print("\n[M2] gold-test permutation importance ...")
    y_for_eval = {a: np.where(vmask, y_axes[a], -1).astype(int) for a in AXES}
    m2 = perm_importance(X, y_for_eval, vmask, gold, feat_names, splits, scope)
    ...
    m4, base = group_ablation(X, y_for_eval, vmask, gold, feat_names, splits, scope)
    ...
    # pivot M1 / M2 noise flags to feature x axis
