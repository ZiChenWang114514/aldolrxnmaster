#!/usr/bin/env python3
"""V4 Model Benchmark: 11 active models × 10 splits (no DRFP — confirmed leakage).
...
def load_full(X, y, feat_names):
    """Full V4 features (all dims)."""
    return X


def load_steric_only(X, y, feat_names):
    """Steric-only features."""
...
def load_conditions_only(X, y, feat_names):
    """Conditions-only features."""
    idx = [i for i, c in enumerate(feat_names) if c.startswith("feat_")]
    return X[:, idx] if idx else X


def load_condaux(X, y, feat_names):
    """Conditions + auxiliary features."""
    idx = [i for i, c in enumerate(feat_names)
           if c.startswith("feat_") or c.startswith("aux_") or c == "n_defined_stereocenters"]
    return X[:, idx] if idx else X


def load_steric_cond_aux(X, y, feat_names):
    """Steric + conditions + auxiliary (full minus nothing)."""
    return X


def load_condaux_chiral(X, y, feat_names):
    """Conditions + auxiliary + chirality + rgroup features (no steric, no chiralenv)."""
    idx = [i for i, c in enumerate(feat_names)
...
def _load_mechaware():
    ...
    if _MA_FULL is None:
        ma_path = FEAT_DIR.parent / "features_v4" / "v4_mechaware_full.csv"
        if ma_path.exists():
        ...
        bw_path = FEAT_DIR.parent / "features_v4" / "v4_mechaware_bw.csv"
        if bw_path.exists():
...
def load_ma_full_plus(X, y, feat_names):
    """MechAware-Full (328d) + chirality/rgroup/aldpri features from V4b."""
    _load_mechaware()
    if _MA_FULL is None:
        return X
...
def load_ma_bw_plus(X, y, feat_names):
    """MechAware-BW (112d) + chirality/rgroup/aldpri features from V4b."""
    _load_mechaware()
    if _MA_BW is None:
        return X
    new_idx = [i for i, c in enumerate(feat_names)
